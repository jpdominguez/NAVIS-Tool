...
namespace NAVIS
{
    ...
    public class aedat32
    {
        ...
        public aedat32(String filePath)
        {
            ...
            #endregion
            #region .csv
            else if (MainWindow.fileName.Split('.')[MainWindow.fileName.Split('.').Length - 1] == "csv")
            {
        ...
        public bool generateSonogram(String file, int maxValueSonogram)
        {
            ...
            else
            {
                ...
                Bitmap Bmp = new Bitmap(Convert.ToInt32(((maxTimestamp * tam) / MainWindow.settings.ToolsS.integrationPeriod) + 4), frameSizeHeight * tam);
                Color color;
                int contador = 0;
                int eventos = 0;
        ...
        /// </summary>
        public void AedatSplitterAuto(int noiseTolerance, int noiseThreshold)
        {
            int vecesQueEntra = 0;
            int eventsFired;
            ...
            for (long i = 0; i <= maxTimestamp; i += (int)MainWindow.settings.ToolsS.integrationPeriod)
            {
                ...
                if (eventsFired < noiseThreshold * 0.0001 * lista.Count)
                {
                    ...
                    if (haEntrado == true)
                    {
                        ...
                        if (vecesQueEntra < noiseTolerance)
                        {
                            aedatTroceado[indiceTrozo - 1] = null;
                        }
                        vecesQueEntra = 0;
                    }
                }
            ...
            foreach (List<aedatEvent32> r in aedatTroceado)
            {
                if (r != null)
                {
                    saveAedat("./" + MainWindow.fileName.Split('.')[0] + "/split" + a + ".aedat", r);
                    a++;
                }
            }
        }
